...
3. **Start the server:**
   ```bash
   conda run -n sys3 python -m src.server
   ```
...
├── log_viewer/             # Experiment visualization tool
│   ├── server.py           # Standalone HTTP server (port 8000)
│   └── index.html          # Google Maps-based experiment viewer
├── data/                   # Data files
...
The log viewer is a standalone tool for visualizing experiment logs. To use it:

1. **Copy `log_viewer/` contents** into your experiment logs directory
2. **Set `GOOGLE_MAPS_API_KEY`** (in your environment or a `.env` next to `server.py`)
3. **Run the server:**
   ```bash
   conda run -n sys3 python server.py
   ```
4. **Open** `http://localhost:8000` in your browser

Features:
- Path visualization with heatmap option
- API call decision point inspection
- Success/failure status tracking
- Multi-experiment browsing
